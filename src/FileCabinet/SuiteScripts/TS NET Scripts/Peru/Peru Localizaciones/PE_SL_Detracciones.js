...
define(['N/ui/serverWidget', 'N/email', 'N/runtime', 'N/search', 'N/redirect', 'N/task', 'N/log', 'N/file', 'N/encode', 'N/config'],
    ...
    (ui, email, runtime, search, redirect, task, log, file, encode, config) => {

        let items_detraccion = ",19,1955,1946,"
        const onRequest = (context) => {
            //const file_cabinet_detrac_id = 415;
            const file_cabinet_detrac_id = 646;//MISAKI
            try {
                if (context.request.method === 'GET') {
                    ...
                    if (filterPage == '3' || filterPage == 3) {
                        ...
                        log.debug('MSK', 'A punto de llamar a customsearch_pe_gene_logs_detraction')
                        var mySearch = search.load({
                            id: 'customsearch_pe_gene_logs_detraction' //! Saved Search: 2521 - PE Generation Logs Detraction
                        });
                        var resultSetPE = mySearch.run();
                        log.debug('MSK', 'Ya llamé a customsearch_pe_gene_logs_detraction')

                        var j = 0;
                        resultSetPE.each(function (result) {
                            // log.debug('MSK','recorriendo resultSetPE')
                        ...
                        log.debug('MSK', 'A punto de consumir customsearch_pe_accounts_bankings')
                        var mySearch = search.load({
                            id: 'customsearch_pe_accounts_bankings' //! Saved Search: 2522 - PE Accounts Bankings
                        });
                        var resultSetPE = mySearch.run(); //.getRange({start: pageId*10,end: pageId*10+10});
                        log.debug('MSK', 'Despues de consumir customsearch_pe_accounts_bankings')
                    ...
                    if (filterPagePOST == '2' || filterPagePOST == 2) {
                        ...
                        redirect.toSuitelet({
                            /*scriptId: 209 ,
                            deploymentId: 1,*/
                            //  scriptId: 'customscript_pe_suitelet_detraccion_2',//MISAKI
                            //  deploymentId: 'customdeploy_pe_suitelet_detracciones_ge',
                            scriptId: 'customscript_pe_sl_detracciones_gen',
                    ...
                    log.debug('MSK', 'filterPagePOST = ' + filterPagePOST + ' (opcion 2)')//MISAKI
