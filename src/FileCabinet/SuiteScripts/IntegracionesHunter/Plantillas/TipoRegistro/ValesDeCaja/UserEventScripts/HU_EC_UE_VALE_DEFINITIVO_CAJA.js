        ...
        const beforeLoad = (scriptContext) => {
            try{
                if (scriptContext.type == scriptContext.UserEventType.VIEW) {
                    ...
                    var id = objRecord.id;
                    var type = 'expensereport'
                    form.addButton({
                        id: "custpage_custom_button",
                        label: "Imprimir Vale",
                        functionName: `customButtonFunction('${type}','${id}')`
                    });
                    form.clientScriptModulePath = 'SuiteScripts/IntegracionesHunter/Plantillas/TipoRegistro/ValesDeCaja/ClientScripts/HU_EC_CS_VALE_DEFINITIVO_CAJA.js'
                }
            }catch(error){
