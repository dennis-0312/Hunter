            ...
            try {
                ...
                if (lotData[0].inputfile) {
                    ...
                    if (responseProcesssData.pinicial == 0) {
                        ...
                        record.submitFields({
                            type: 'invoice',
                            id: scriptParameters.facturaDirectaid,
                            values: {
                                custbody_ht_status_process_group: responseProcesssData.statusProcess
                            }
                        });
                    }
                } else {
                ...
                for (let i = puntoInicial; i < ordenID.length; i++) {
                    ...
                    if (scriptObj.getRemainingUsage() > 100) {
                        ...
                        try {
                            ...
                            if (ordenID[i].creado_desde) { record.submitFields({ type: 'salesorder', id: ordenID[i].creado_desde, values: { custbody_ht_factura_directa: facturaDirecta } }) };
                            ...
                            if (ordenID[i].creado_desde) { factura.setValue({ fieldId: 'custrecord_orden_servicio', value: ordenID[i].creado_desde }) }
