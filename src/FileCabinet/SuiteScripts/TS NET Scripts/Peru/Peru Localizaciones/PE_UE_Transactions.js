 ...
 */
define(['N/log', 'N/search', 'N/record', 'N/runtime', 'N/config'],
    (log, search, record, runtime, config) => {
        ...
        const PE_DOCUMENT_TYPE_CREDIT_MEMO = 53
        ...
        //Inicio Cambio 13/06/2024 Jechevarria
        //Formularios de origen excluidos del desarrollo
        const FORM_PE_FACTURA_DE_VENTA = 183; //SB: 205 - PR: 183
        const FORM_PE_NOTA_DE_CREDITO = 184; //SB: 206 - PR: 184
        //fin Cambio 13/06/2024 Jechevarria

        //& <I> - dfernandez - 02/12/2024
        let FEL_TEMPLATE = "";
        let FEL_SENDING_METHOD = ""
        ...
        const configEnviroment = () => {
            ...
            if (EN_SANDBOX) {
                variables.custbody_psg_ei_template = 4;
                variables.custbody_psg_ei_sending_method = 6;
            } else {//EN_PROD
                variables.custbody_psg_ei_template = 4;
                variables.custbody_psg_ei_sending_method = 6;
            }
        ...
        const beforeLoad = (context) => {
            ...
            //Inicio Cambio 13/06/2024 Jechevarria
            let form = context.newRecord.getValue({ fieldId: 'customform' });
            if (form == FORM_PE_FACTURA_DE_VENTA || form == FORM_PE_NOTA_DE_CREDITO) {
                            ...
                            if (objRecord.type == CREDIT_MEMO) {
                                ...
                                if (createdFromTransaction) {
                                    ...
                                    let invoiceSearchObj = search.create({
                                        type: "invoice",
                                        filters: [["type", "anyof", "CustInvc"], "AND", ["mainline", "is", "T"], "AND", ["internalid", "anyof", createdFromTransaction]],
                                        columns: [search.createColumn({ name: "trandate", label: "Date" })]
                                    });
                                    invoiceSearchObj.run().each(function (result) {
                                        dateRef = result.getValue('trandate');
                                    });
                                    ...
                                    objRecord.setValue({ fieldId: 'custbody_pe_document_date_ref', value: dateRef, ignoreFieldChange: true });
                                    objRecord.setValue({ fieldId: 'custbody_pe_document_type', value: PE_DOCUMENT_TYPE_CREDIT_MEMO, ignoreFieldChange: true });
        ...
        const beforeSubmit = (context) => {
            ...
            //log.error('varEnviroment', varEnviroment);
            FEL_TEMPLATE = varEnviroment.custbody_psg_ei_template;
            FEL_SENDING_METHOD = varEnviroment.custbody_psg_ei_sending_method;
            //**============================================================================================================ */
            //& <F> - dfernandez - 02/12/2024
            //Inicio Cambio 13/06/2024 Jechevarria
            let form = objRecord.getValue({ fieldId: 'customform' });
            if (form == FORM_PE_FACTURA_DE_VENTA || form == FORM_PE_NOTA_DE_CREDITO) {
                //& <I> - dfernandez - 02/12/2024
                if (eventType === context.UserEventType.CREATE || eventType === context.UserEventType.COPY) {
                    if (type == record.Type.INVOICE) {
                        try {
                            log.error('varEnviroment', varEnviroment);
                            objRecord.setValue({ fieldId: 'custbody_psg_ei_template', value: FEL_TEMPLATE, ignoreFieldChange: true });
                            objRecord.setValue({ fieldId: 'custbody_psg_ei_status', value: For_Generation_Status, ignoreFieldChange: true });
                            objRecord.setValue({ fieldId: 'custbody_psg_ei_sending_method', value: FEL_SENDING_METHOD, ignoreFieldChange: true });
                        } catch (error) {
                            log.error('Error-beforeSubmit-E-DOC', error);
                        }
                    }
                }
                //& <F> - dfernandez - 02/12/2024
                ...
                try {
                    if (eventType === context.UserEventType.CREATE || eventType === context.UserEventType.COPY) {
                        ...
                        if (type == record.Type.INVOICE) {
                            ...
                            if (peTipoDocumento == '') {
                                objRecord.setValue({ fieldId: 'custbody_pe_document_type', value: 50, ignoreFieldChange: true });
                            }
                            if (peConceptoDetra == '') {
        ...
        const updateTransactionTranId = (type, id, tranid, peNumber, peSerie) => {
            ...
            let recordId = record.submitFields({
                ...
                values,
                options: {
                    ignoreMandatoryFields: true // Ignora campos obligatorios
                }
            });
            log.debug('updateTransactionTranId Datos actualizados', {
                type: type,
                id: recordId,
                tranid: tranid,
                peNumber: peNumber,
        ...
        const afterSubmit = (context) => {
            ...
            if (form == FORM_PE_FACTURA_DE_VENTA || form == FORM_PE_NOTA_DE_CREDITO) {
                if (form == FORM_PE_FACTURA_DE_VENTA) {
                    if (eventType === context.UserEventType.CREATE) {
                        try {
                            let get_current_pe_serie = objRecord.getValue({ fieldId: 'custbody_pe_serie_2_1' });
                            let get_current_pe_pe_number = objRecord.getValue({ fieldId: 'custbody_pe_number' });
                            ...
                            let serie = objRecord.getValue({ fieldId: 'custbody_pe_serie' });
                            ...
                            let getserie = getSerie(codeDocumentType, location, prefix, subsidiary, documentReferenceType, serie);
                            log.error("getserie", getserie);
                            let correlativeObj = generateCorrelative(getserie.peinicio, getserie.serieid, getserie.serieimpr);
                            //actualizamos el tranid
                            // let new_tranid = 'FA-PE-' + get_current_pe_serie + '-' + get_current_pe_pe_number;
                            // let new_tranid = 'FA-PE-' + correlativeObj.numbering;
                            let new_tranid = 'PE-' + correlativeObj.numbering;
                            record.submitFields({
                                type: objRecord.type,
                                id: objRecord.id,
                                values: {
                                    'tranid': new_tranid,
                                    'custbody_pe_number': correlativeObj.correlative, // add
                                }
                            });
                        } catch (error) {
                ...
                if (form == FORM_PE_NOTA_DE_CREDITO) {
                    if (eventType === context.UserEventType.CREATE) {
                        try {
                            let get_current_pe_serie = objRecord.getValue({ fieldId: 'custbody_pe_serie_2_1' });
                            let get_current_pe_pe_number = objRecord.getValue({ fieldId: 'custbody_pe_number' });
                            //actualizamos el tranid
                            //<I> rhuaccha: 2024-10-02
                            ...
                            log.debug('prefSubsi', prefSubsi);
                            let documentReferenceType = "";
                            if (codeDocumentType == "08" || codeDocumentType == "07") {
                                documentReferenceType = record.Type.INVOICE;
                                documentReferenceType = objRecord.getValue('custbody_pe_document_type_ref');
                            }

                            let serie = objRecord.getValue('custbody_pe_serie');

                            let getserie = getSerie(codeDocumentType, location, prefix, subsidiary, documentReferenceType, serie);
                            log.error("SERIE", getserie);
                            let correlative = generateCorrelative(getserie.peinicio, getserie.serieid, getserie.serieimpr);
                            log.error("CORRELATIVO", correlative);
                            transactionTranId = prefSubsi + correlative.numbering;
                            log.debug('transactionTranId', transactionTranId);
                ...
                if (eventType === context.UserEventType.CREATE) {
                    ...
                    if (type === record.Type.INVOICE || type === record.Type.CREDIT_MEMO || type === record.Type.CASH_SALE) {
                        if (codeDocumentType != "") {
                            ...
                            let getserie = getSerie(codeDocumentType, location, prefix, subsidiary, documentReferenceType, 0);
                            log.error("getserie", getserie);
                            let correlative = generateCorrelative(getserie.peinicio, getserie.serieid, getserie.serieimpr);
            ...
            if (eventType === context.UserEventType.CREATE || eventType === context.UserEventType.EDIT) {
                let recordId = objRecord.id;
                recordLoadinvoice = record.load({ type: objRecord.type, id: recordId, isDynamic: false });
                let currencyname = recordLoadinvoice.getText({ fieldId: 'currency' });
                let custbody_pe_concept_detraction = recordLoadinvoice.getValue({ fieldId: 'custbody_pe_concept_detraction' });
                let form2 = recordLoadinvoice.getValue({ fieldId: 'customform' });

                log.debug('custbody_pe_concept_detraction', custbody_pe_concept_detraction);
                log.debug('form', form2);

                if (custbody_pe_concept_detraction != 1 && form2 == 183) {
                    log.debug('entro');
                    recordLoadinvoice.setValue({ fieldId: 'custbody_4601_appliesto', value: 'T' });
                    recordLoadinvoice.save({ enableSourcing: true, ignoreMandatoryFields: true })
                }
            }
        ...
        const afterSubmit1 = (context) => {
            ...
            var FORM_DEBIT_MEMO = runtime.getCurrentScript().getParameter({ name: 'custscript_pe_ue_transaction_formulario' });
            if (eventType === context.UserEventType.CREATE) {
        ...
        const getSerie = (documenttype, location, prefix, subsidiaryId, documentref = 0, serie) => {
            let searchLoad = '';
            try {
                log.debug('DebugSearch1', documenttype + ' - ' + location + ' - ' + prefix + ' - ' + documentref + ' - ' + serie)
                if (documenttype == DOCUMENT_TYPE_CREDIT_MEMO || documenttype == DOCUMENT_TYPE_DEBIT_MEMO) {
                    log.debug('Debug', 'Entre a search NC o ND');
                    if (serie != 0 && serie.length > 0) {
                        searchLoad = search.create({
                            type: 'customrecord_pe_serie',
                            filters: [
                                ['internalid', 'anyof', serie],
                                'AND',
