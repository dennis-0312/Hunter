...
define(['N/search', 'N/record', 'N/runtime', 'N/log', 'N/file', 'N/task', "N/config"], (search, record, runtime, log, file, task, config) => {
    // Schedule Report: PE - Anual: Inv. Balance - Detalle 34 - 3.9
    const execute = (context) => {
        try {
            const featureSubsidiary = runtime.isFeatureInEffect({ feature: 'SUBSIDIARIES' });
            const searchId = 'customsearch_pe_detalle_34_3_9';
    ...
    const getParams = () => {
        try {
            ...
            const filterSubsidiary = scriptObj.getParameter({ name: 'custscript_pe_ss_ple_3_9_invbal_sub' });
            const filterPostingPeriod = scriptObj.getParameter({ name: 'custscript_pe_ss_ple_3_9_invbal_per' });
            const filterFormat = scriptObj.getParameter({ name: 'custscript_pe_ss_ple_3_9_invbal_for' });
            const fileCabinetId = scriptObj.getParameter({ name: 'custscript_pe_ss_ple_3_9_invbal_fol' });
            const filterAnioPeriod = scriptObj.getParameter({ name: 'custscript_pe_ss_ple_3_9_invbal_year' });
            return {
    ...
    const createRecord = (logrecodId, featureSubsidiary, filterSubsidiary, filterPostingPeriod) => {
        try {
            ...
            recordlog.setValue({ fieldId: 'custrecord_pe_book_log', value: 'Inventario y Balance 3.9' });
...
/*                         column01 = column01.split('/');
                        ...
                        if (column10 == anioperiod) {
                            json.push({
                                ...
                                c4_fecha_op: column04,
                                c5_cod_cuenta_contable: column05,
                                c6_desc_intangible: column06,
                                c7_valor_contable: column07,
                                c8_amortizacion_contable: column08,
...
/*                             column01 = column01.split('/');
        ...
        try {
            for (let i in searchResult) {
                contentReport =
                    ...
                    searchResult[i].c4_fecha_op + '|' + searchResult[i].c5_cod_cuenta_contable + '|' + searchResult[i].c6_desc_intangible + '|' +
                    searchResult[i].c7_valor_contable + '|' + searchResult[i].c8_amortizacion_contable + '|' + searchResult[i].c9_estado_op + '|\n';
            }
    ...
    const createFile = (filterPostingPeriod, fedIdNumb, hasinfo, recordlogid, filterFormat, structuregbody, fileCabinetId,filterAnioPeriod) => {
        let typeformat;
        const header = '1 Periodo|2 CUO|3 Correlativo|4 Fecha de la operacion o emision|5 Codigo de la cuenta contable asociada a la obligacion|6 Descripcion del intangible|' +
            '7 Valor contable del intangible|8 Amortizacion contable acumulada|9 Indica el estado de la operacion|\n';
        try {
            
            let periodname = filterAnioPeriod ;
            log.debug('structuregbody',structuregbody)
            let nameReportGenerated = 'LE' + fedIdNumb + periodname + '1231030900' + '07' + '1' + hasinfo + '11_' + recordlogid;
            if (filterFormat == 'CSV') {
