 ...
 * 3.4           28 Ago 2023     Ivan Morales <imorales@myevol.biz>             - Creación del reporte 3.4
 *
 */
...
define(['N/runtime', 'N/search', 'N/config', 'N/render', 'N/record', 'N/file', './PE_LIB_Libros.js', 'N/format'], (runtime, search, config, render, record, file, libPe, format) => {
    ...
    const getFileName = () => {
        return `LE${companyRuc}${pGloblas.pAnio}1231030400071${hasInfo}11_${pGloblas.pRecordID}.pdf`;
    }
    ...
    const summarize = (context) => {
        ...
        log.error("periodId", periodId);
        pGloblas['pRecordID'] = libPe.createLog(pGloblas.pSubsidiary, periodId, "Inventario y Balance 3.4")
    ...
    const getJsonData = (transactions) => {
        ...
        for (var k in transactions) {
            ...
            if (!jsonTransacion[IDD]) {
                jsonTransacion[IDD] = {
                    ...
                    montoCtaCobrar: Number(transactions[k].montoCtaCobrar).toFixed(2),
            ...
            if(cantidadLinea <= 26){
                ...
                jsonTransacionvine.push({
                    ...
                    //montoviene : montoviene.toFixed(2),
                    cantidadFor :cantidadFor-1
                });
        ...
        let jsonAxiliar = {
            "company": {
                ...
                "thirdTitle": 'FORMATO 3.4 - ' + pGloblas.Anio,
            },
            "cabecera": {
                "Anio": pGloblas.pAnio,
                "ruc": companyRuc,
    ...
    const getTemplate = () => {
        var aux = file.load("./Template/PE_Template_Formato_3_4.ftl");
    ...
    const getTransactions = () => {
        ...
        var savedSearch = search.load({
            id: 'customsearch_pe_libro_impreso_3_4'
        });
        ...
        pagedData.pageRanges.forEach(function (pageRange) {
            ...
            page.data.forEach(function (result) {
              ...
              // 3. APELLIDOS Y NOMBRES, DENOMINACIÓN O RAZÓN SOCIAL
                ...
                let auxcolumn04 = arrAux[3].replace(/\s/g, '');
                if (auxcolumn04 == '-None-'|| auxcolumn04.indexOf('ERROR:') >= 0) {
                  arrAux[3] = ''
                }
                ...
                log.debug('ERROR', 'arrAux[7] = '+arrAux[7] );
                //Temporal

                let IDEntidad = result.getValue(columns[8]);
                let nombreCuenta = result.getValue(columns[9]);
                let numCuenta = result.getValue(columns[10]);

                let nombre = nombreCuenta;
                let numTrabajador = numCuenta;
    ...
    const getParameters = () => {
        pGloblas = objContext.getParameter('custscript_pe_formato_3_4_params');
