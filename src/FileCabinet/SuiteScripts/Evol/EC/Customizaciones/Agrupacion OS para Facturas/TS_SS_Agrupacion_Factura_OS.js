...
define(['N/record', 'N/log', 'N/search', 'N/format', 'N/runtime', 'N/task', 'N/file'],
    ...
    (record, log, search, format, runtime, task, file) => {
        const FORM_EC_FORMULARIO_FACTURA_VENTA = 101;//SANDBOX = 101, PRODUCCION = 101
        const TIPO_DOCUMENTO_18 = 16; //SANDBOX = 16, PRODUCCION = 16
        ...
        const generarFacturaDirecta = (params, id_cola) => {
            ...
            try {
                ...
                //Actualizamos el porcentaje del log
                try {
                    updateCabLog({ idCabLog: idCabLog, porcentaje: '100%', estado: 'Error', fechaFin: true, errores: error.message });
        ...
        const taskScheduled = (objSendData) => {
            try {
                ...
                record.submitFields({ type: LOG_RECORD, id: objSendData.id, values: { custrecord_ts_ss_estado_asociar_fac: PENDIENTE } });
            }
        }
