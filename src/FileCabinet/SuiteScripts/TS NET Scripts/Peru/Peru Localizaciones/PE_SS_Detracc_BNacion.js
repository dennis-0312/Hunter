...
define(['N/search', 'N/record', 'N/email', 'N/runtime', 'N/log', 'N/file', 'N/task', "N/config", "N/format"],
    (search, record, email, runtime, log, file, task, config, format) => {
        ...
        const execute = (context) => {
            ...
            var fileCabinetId = scriptObj.getParameter({ name: 'custscript_pe_ss_detracc_bnacion_file' });

            try {
                log.debug({ title: 'INICIO', details: 'INICIANDO SCHEDULED BANCO DE LA NACION: ' + fedIdNumb + ' - ' + fedIdNumb });
                ...
                log.debug({ title: 'myObj', details: 'FINALIZANDO DETRACCIONES BANCO NACION, FILE GENERADO ' + fileId });
                var customRecord = record.create({ type: 'customrecord_pe_generation_logs', isDynamic: true });
                if (featureSubsidiary || featureSubsidiary == 'T') {
                    var nameData = {
                ...
                log.debug('scriptTaskId2', scriptTaskId2)
            } catch (e) {
