        ...
        if (context.type !== context.UserEventType.EDIT) {
            try {
                var GetCustomer = https.get({
                    url: "https://telematicsapi.hunterlabs.io/asset/?user=" + "STRING",
                    headers: headers1
                });
                GetCustomer = JSON.parse(GetCustomer.body);
                headers1['X-HTTP-Method-Override'] = 'PATCH';
                var respasset = https.put({
                    url: "https://telematicsapi.hunterlabs.io/asset/" + GetCustomer.id + "/",
                    headers: headers1,
                    body: JSON.stringify({
        ...
        if (context.type !== context.UserEventType.CREATE) {
            try {
                ...
                var respAsset = https.post({
                    url: "https://telematicsapi.hunterlabs.io/asset/",
                    headers: headers1,
                    body: JSON.stringify({
                        "product": "PruebaEvol",
                        "name": "PruebaEvol",
