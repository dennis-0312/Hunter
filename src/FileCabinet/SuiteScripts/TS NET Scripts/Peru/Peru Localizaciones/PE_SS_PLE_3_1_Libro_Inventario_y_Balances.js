...
define(['N/search', 'N/record', 'N/runtime', 'N/log', 'N/file', 'N/task', "N/config"], (search, record, runtime, log, file, task, config) => {
    // Libro de Inventario y Balances - Estado de Situación Financiera - 3.1
    const execute = (context) => {
        try {
            const featureSubsidiary = runtime.isFeatureInEffect({ feature: 'SUBSIDIARIES' });
            const searchId = 'customsearch_pe_detalle_3_1'; // PE - Libro de Inventario y Balances - Estado de Situación Financiera - 3.1
    ...
    const getParams = () => {
        try {
            ...
            const filterSubsidiary = scriptObj.getParameter({ name: 'custscript_pe_ss_ple_3_1_invbal_sub' });
            const filterPostingPeriod = scriptObj.getParameter({ name: 'custscript_pe_ss_ple_3_1_invbal_per' });
            const filterFormat = scriptObj.getParameter({ name: 'custscript_pe_ss_ple_3_1_invbal_for' });
            const fileCabinetId = scriptObj.getParameter({ name: 'custscript_pe_ss_ple_3_1_invbal_fol' });
            const filterAnioPeriod = scriptObj.getParameter({ name: 'custscript_pe_ss_ple_3_1_invbal_year' });
            log.debug('AÑO', filterAnioPeriod);
            return {
    ...
    const createRecord = (logrecodId, featureSubsidiary, filterSubsidiary, filterPostingPeriod) => {
        try {
            ...
            recordlog.setValue({ fieldId: 'custrecord_pe_book_log', value: 'Inventario y Balance 3.1' });
                ...
                if (searchResultCount <= 4000) {
                    searchLoad.run().each((result) => {
                      ...
                      column04 = numberWithCommas(column04);
                        ...
                        log.debug('COL06', column06 + ' - ' + anioperiod);
        ...
        try {
            for (let i in searchResult) {
                contentReport =
                    ...
                    searchResult[i].c4_saldo_rubro + '|' + searchResult[i].c5_estado_operacion + '|\n';
            }
    ...
    const createFile = (filterPostingPeriod, fedIdNumb, hasinfo, recordlogid, filterFormat, structuregbody, fileCabinetId,filterAnioPeriod) => {
        ...
        try {
            ...
            let nameReportGenerated = 'LE' + fedIdNumb + periodname + '1231030100' + '07' + '1' + hasinfo + '11_' + recordlogid;
            if (filterFormat == 'CSV') {
    ...
    // const getPeriodName = (filterAnioPeriod) => {
    //     try {
