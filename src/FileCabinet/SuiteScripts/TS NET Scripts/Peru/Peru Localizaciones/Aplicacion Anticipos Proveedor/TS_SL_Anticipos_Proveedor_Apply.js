    ...
    (config, file, log, query, record, redirect, runtime, search, task, dialog, message, serverWidget, _config) => {
        ...
        const processData = (scriptContext) => {
            ...
            try {
                ...
                const end = Math.min(offset + limit, batchData.length);
                const batchToProcess = batchData.slice(offset, end);

                const results = procesarDatosSeleccionados(batchToProcess);

                scriptContext.response.write(JSON.stringify({
                    success: true,
                    processed: results.processed,
                    totalRecords: batchData.length,
                    processedSoFar: offset + results.processed,
                    nextOffset: end < batchData.length ? end : null,
                    message: `Procesados ${offset + results.processed} de ${batchData.length} registros`
                }));
        ...
        const procesarDatosSeleccionados = (data, offset = 0, limit = data.length) => {
            ...
            for (let i = offset; i < end; i++) {
                ...
                try {
                    ...
                    vendorBill.setValue({ fieldId: 'account', value: account });
