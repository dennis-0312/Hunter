...
<#if payment.currency == "US Dollar">
<#assign tranAmount = tranAmount * payment.exchangerate>
</#if>
<#return tranAmount>
</#function>
...
<#assign totalAmount = computeTotalAmount(payments, true)>
...
<#assign ebank = ebanks[payment_index] >
<#assign amountPayment = getAmount(payment, true)>
...
3${tipoTransaccion}${setPadding(transaction.tranid?replace(' ','')?replace('-',''),"left","0",15)}${setPadding(formatAmount(getReferenceAmount(payment, pfa.custrecord_2663_account.type, transaction),"dec"),"left","0",17)}
</#list>
</#list>
#OUTPUT END#