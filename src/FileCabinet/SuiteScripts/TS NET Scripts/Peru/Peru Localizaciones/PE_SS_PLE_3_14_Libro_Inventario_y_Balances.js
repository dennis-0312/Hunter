...
define(['N/search', 'N/record', 'N/runtime', 'N/log', 'N/file', 'N/task', "N/config"], (search, record, runtime, log, file, task, config) => {
    // Schedule Report: PE - Anual: Inv. Balance - Detalle 47 - 3.14
    const execute = (context) => {
        try {
            const featureSubsidiary = runtime.isFeatureInEffect({ feature: 'SUBSIDIARIES' });
            const searchId = 'customsearch_pe_detalle_47_3_14'; // PE - Libro de Inventario y Balances - Detalle del Saldo de la Cuenta 47 - 3.14
    ...
    const getParams = () => {
        try {
            ...
            const filterSubsidiary = scriptObj.getParameter({ name: 'custscript_pe_ss_ple_3_14_invbal_sub' });
            const filterPostingPeriod = scriptObj.getParameter({ name: 'custscript_pe_ss_ple_3_14_invbal_per' });
            const filterFormat = scriptObj.getParameter({ name: 'custscript_pe_ss_ple_3_14_invbal_for' });
            const fileCabinetId = scriptObj.getParameter({ name: 'custscript_pe_ss_ple_3_14_invbal_fol' });
            const filterAnioPeriod = scriptObj.getParameter({ name: 'custscript_pe_ss_ple_3_14_invbal_year' });
            return {
    ...
    const createRecord = (logrecodId, featureSubsidiary, filterSubsidiary, filterPostingPeriod) => {
        try {
            ...
            recordlog.setValue({ fieldId: 'custrecord_pe_book_log', value: 'Inventario y Balance 3.14' });
            ...
            if (searchResultCount != 0) {
                if (searchResultCount <= 4000) {
                    searchLoad.run().each((result) => {
                        ...
                        column09 = column09.replace(/(\r\n|\n|\r)/gm, "");
                        column09 = column09.replace(/[\/\\|]/g, "");
                        ...
                        let numTrabajador = entidad[1];
                        let isPerson = entidad[2];
                        let altname = entidad[3];
                        let codigoTipoDocumento = entidad[4];
                        ...
                        if (column10 == anioperiod) {
                            json.push({
                                ...
                                //c4_tipo_doc_trabajador: column04,
                                //c5_nro_doc_trabajador: column05,
                                //c6_nombres_trabajador: column06,
                                c4_tipo_doc_trabajador: codigoTipoDocumento,
                                c5_nro_doc_trabajador: numTrabajador,
                                c6_nombres_trabajador: nombre,
                                c7_saldo_final: column07,
                                c8_estado_op: column08,
                                c9_cuenta_asociada: column09,
                            });
                        }
                        return true;
                    });
        ...
        try {
            for (let i in searchResult) {
                contentReport =
                    ...
                    searchResult[i].c4_tipo_doc_trabajador + '|' + searchResult[i].c5_nro_doc_trabajador + '|' + searchResult[i].c6_nombres_trabajador + '|' +
                    searchResult[i].c7_saldo_final + '|' + searchResult[i].c8_estado_op + '|' + searchResult[i].c9_cuenta_asociada + '|\n';
            }
    ...
    const createFile = (filterPostingPeriod, fedIdNumb, hasinfo, recordlogid, filterFormat, structuregbody, fileCabinetId, filterAnioPeriod) => {
        let typeformat;
        const header = '1 Periodo|2 CUO|3 Correlativo|4 Tipo de Documento de Identidad del trabajador|5 Numero de Documento de Identidad del trabajador|6 Apellidos y Nombres del trabajador|' +
            '7 Saldo Final|8 Indica el estado de la operacion|	9 Cuenta asociada|\n';
        try {
            let periodname = filterAnioPeriod;
            let nameReportGenerated = 'LE' + fedIdNumb + periodname + '1231031400' + '07' + '1' + hasinfo + '11_' + recordlogid;
            if (filterFormat == 'CSV') {
