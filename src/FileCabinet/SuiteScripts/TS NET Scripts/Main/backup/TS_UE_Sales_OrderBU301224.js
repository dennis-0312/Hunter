...
define([
    ...
    const beforeLoad = (scriptContext) => {
        ...
        if (scriptContext.type === scriptContext.UserEventType.COPY) {
            scriptContext.newRecord.setValue('custbody_ht_so_renovacion_aplicada', false);
        }
    }
        ...
        if (scriptContext.type === scriptContext.UserEventType.EDIT || scriptContext.type === scriptContext.UserEventType.CREATE || scriptContext.type === scriptContext.UserEventType.COPY) {
            ...
            try {
                ...
                */
                ...
                if (scriptContext.type === scriptContext.UserEventType.CREATE || scriptContext.type === scriptContext.UserEventType.COPY
                    || (scriptContext.type === scriptContext.UserEventType.EDIT && objRecord.getValue('custbody_ht_so_renovacion_aplicada') == false /*&& userId == 4*/)) {
                    for (let i = 0; i < numLines; i++) {
                        //Validacion 1
                        let validacion1 = false;

                        let validacionDescuento = objRecord.getSublistValue({ sublistId: 'item', fieldId: 'rate', line: i });//Precion UNitario del Articulo
                        let validacionGarantia = true; // Por el momento no hay campo en la orden de venta que valide la garantia
                        let validacionNivelPrecio = objRecord.getSublistValue({ sublistId: 'item', fieldId: 'custcol_ht_os_nivelprecio', line: i });//Nivel de Precio
                        ...
                        log.error('CoberturaSinOT', `${validacionDescuento} != 0 && ${validacionGarantia} && ${validacionNivelPrecio} != 'Personalizado' && ${validacionAPRSolicitaAprobacion}`);
                        if (/*validacionDescuento != 0 && validacionGarantia &&*/ validacionNivelPrecio != 'Personalizado' && validacionAPRSolicitaAprobacion == false) {
                            validacion1 = true;
                        ...
                        log.debug('formula', `${validacionLimiteCredito} - ${validacionOrdenesNoFacturadas} - ${validacionSaldoVencido}`)
                        ...
                        if (validacion1 && validacion2 || (scriptContext.type === scriptContext.UserEventType.EDIT && aprobacionventaCobertura == _constant.Status.APROBADO && aprobacioncarteraCobertura == _constant.Status.APROBADO)) {
                            try {
                                let articulo = objRecord.getSublistValue({ sublistId: 'item', fieldId: 'item', line: i });
                                let cos = _controller.getParameter(articulo, _constant.Parameter.COS_CIERRE_DE_ORDEN_DE_SERVICIO);
                                if (cos != 0 && cos == _constant.Valor.SI) {
                                    cerrarOrden = 1
                                }
                                let articulo_type = objRecord.getSublistValue({ sublistId: 'item', fieldId: 'itemtype', line: i });
                                ...
                                log.debug('parametrosArticulo', { articulo: articulo, articulo_type: articulo_type, parametrosArticulo: parametrosArticulo });
                                if (articulo_type == 'Service' && parametrosArticulo == '008') {
                                    let sql = "SELECT count(*) as cantidad FROM customrecord_ht_co_cobertura WHERE custrecord_ht_co_bien = ? AND custrecord_ht_co_producto = ?";
                                    let parametros = [scriptContext.newRecord.getValue('custbody_ht_so_bien'), articulo];
                                    let results = query.runSuiteQL({ query: sql, params: parametros }).asMappedResults();
                                    log.debug('asMappedResults', `${results[0].cantidad}`);
                                    if (results[0].cantidad == 0) {
                                        let params = {};
                                        params.plazo = objRecord.getSublistValue({ sublistId: 'item', fieldId: 'custcol_ht_os_tiempo_cobertura', line: i });
                                        params.fechaInicio = scriptContext.newRecord.getValue('trandate');
                                        let cobertura = getCobertura(params.plazo, objRecord.getSublistValue({ sublistId: 'item', fieldId: 'custcol_ht_os_und_tiempo_cobertura', line: i }), params.fechaInicio, params.fechaInicio).coberturaFinal;
                    ...
                    log.debug('Aprobación-Verify', `${validacionGlobal1} && ${validacionGlobal2} || (${scriptContext.type} === ${scriptContext.UserEventType.EDIT} && ${aprobacionventaCobertura} == ${_constant.Status.APROBADO} && ${aprobacioncarteraCobertura} == ${_constant.Status.APROBADO})`)
                    if (validacionGlobal1 && validacionGlobal2 || (scriptContext.type === scriptContext.UserEventType.EDIT && aprobacionventaCobertura == _constant.Status.APROBADO && aprobacioncarteraCobertura == _constant.Status.APROBADO)) {
                        ...
                        for (let i = 0; i < numLines; i++) {
                            ...
                            htClient = objRecord.getSublistValue({ sublistId: 'item', fieldId: 'custcol_ht_os_cliente', line: i });
                            monitoreo = objRecord.getSublistValue({ sublistId: 'item', fieldId: 'custcol_ht_os_cliente_monitoreo', line: i }).length > 0 ? objRecord.getSublistValue({ sublistId: 'item', fieldId: 'custcol_ht_os_cliente_monitoreo', line: i }) : htClient;
                            bien = objRecord.getValue('custbody_ht_so_bien');
                        ...
                        if (ccd == _constant.Valor.SI || ttr == _constant.Valor.VALOR_CAMB_GPS_TDE_DEALER_MOV_CUSTODIA && (scriptContext.type === scriptContext.UserEventType.CREATE || scriptContext.type === scriptContext.UserEventType.COPY)) {
                            esCustodia = 1
                        }
                        log.debug('Track3', 'Track3');
                        //* CAMBIO DE PROPIETARIO PLATAFORMA ================================================================================================
                        log.debug('CAMBIO DE PROPIETARIO PLATAFORMA', `${parametro} != 0 && ${esCustodia} == 0 && ${esConvenio} == 0 && ${adp} == ${_constant.Valor.VALOR_010_CAMBIO_DE_PROPIETARIO} && (${scriptContext.type} === ${scriptContext.UserEventType.CREATE} || ${scriptContext.type} === ${scriptContext.UserEventType.COPY} || (${scriptContext.type} === ${scriptContext.UserEventType.EDIT} && ${userId} == 4))`)
                        if (parametro != 0 && esCustodia == 0 && esConvenio == 0 && adp == _constant.Valor.VALOR_010_CAMBIO_DE_PROPIETARIO &&
                            (scriptContext.type === scriptContext.UserEventType.CREATE || scriptContext.type === scriptContext.UserEventType.COPY || (scriptContext.type === scriptContext.UserEventType.EDIT && userId == 4))) {
                            ...
                            if (returEjerepo == false) {
                                ...
                                if (busqueda_cobertura != 0) {
                                    for (let i = 0; i < busqueda_cobertura.length; i++) {
                                        ...
                                        let gpg = _controller.getParameter(busqueda_cobertura[i][0], _constant.Parameter.GPG_GENERA_PARAMETRIZACION_EN_GEOSYS);
                                        let gpt = _controller.getParameter(busqueda_cobertura[i][0], _constant.Parameter.GPT_GENERA_PARAMETRIZACION_EN_TELEMATICS);
                                    ...
                                    try {
                                        if (responsePlataformasTM.send) {
                                            let sql = "SELECT count(*) as cantidad FROM customer WHERE id = ? AND custentity_ht_customer_id_telematic = ?";
                                            let params = [htClient, responsePlataformasTM.newCustomer];
                                            let results = query.runSuiteQL({ query: sql, params: params }).asMappedResults();
                                            log.debug('SELECT count(*) as cantidad FROM customer WHERE id = ? AND custentity_ht_customer_id_telematic = ?', results);
                                            if (results[0].cantidad == 0) {
                                                record.submitFields({
                                                    type: 'customer',
                                                    id: htClient,
                                                    values: {
                                                        custentity_ht_customer_id_telematic: responsePlataformasTM.newCustomer
                                                    }
                                                })
                                            }
                                        } else {
                                            log.error('Error-SendTM', 'Se produjo un error, revisar el despliegue customdeploy_ns_rs_new_owner')
                                        }
                                    } catch (error) {
                        ...
                        if (adp == _constant.Valor.VALOR_010_CAMBIO_DE_PROPIETARIO && (scriptContext.type === scriptContext.UserEventType.CREATE || scriptContext.type === scriptContext.UserEventType.COPY)) {
                            if (esCustodia == 1) {
                                let internalid;
                        ...
                        let fechaAprobacion = objRecord.getValue('custbody_ht_fecha_apro');
                        if (!fechaAprobacion)
                        ...
                        if (renovamos == true && objRecord.getValue('custbody_ht_so_renovacion_aplicada') == false) {
                            ...
                            if (results.length > 0) {
                                let vehiculo;
                                if (bien != '') {
                                    vehiculo = search.lookupFields({ type: _constant.customRecord.BIENES, id: bien, columns: ['custrecord_ht_bien_id_telematic', 'name', 'custrecord_ht_bien_placa'] });
                                } else {
                                ...
                                for (let i = 0; i < results.length; i++) {
                                    ...
                                    impulsaPX = _controller.getParameter(results[i].item, _constant.Parameter.GPG_GENERA_PARAMETRIZACION_EN_GEOSYS);
                                    ...
                                    if (results2.length > 0) {
                                        for (let j = 0; j < results2.length; j++) {
                                            ...
                                            let sql3 = "SELECT count(*) as larenovacionyaseaplico FROM customrecord_ht_ct_cobertura_transaction " +
                                                "WHERE custrecord_ht_ct_orden_servicio = ? AND custrecord_ht_ct_concepto = ? AND custrecord_ht_ct_transacciones = ?"
                                            let params3 = [idRecord, _constant.Status.RENOVACION, idCoberturaItem];
                                            let results3 = query.runSuiteQL({ query: sql3, params: params3 }).asMappedResults();
                                            log.debug('results3.runSuiteQL.VerificarSiLaRenovacionYaSeAplico', results3);
                                            if (results3[0].larenovacionyaseaplico == 0) {
                                                ...
                                                if ((plataformas == true || impulsaPX == true) && t_PPS == true) {
                                                    ...
                                                    if (newDateAntigua < hoy) { objValues.custrecord_ht_co_coberturafinal = new Date(fechaNuevaDia) }
                                                    objValues.custrecord_ht_co_coberturafinal = new Date(fechaNuevaCompleta)
                                                    objValues.custrecord_ht_co_plazo = newDateAntigua < hoy ? plazo : plazoTotal
                                                    objValues.custrecord_ht_co_estado_cobertura = _constant.Status.ACTIVO
                                                    // if (impulsaPX == true) {
                                                    //     log.debug('impulsaPX', impulsaPX)
                                                    // let pxAdmin = {
                                                    ...
                                                    if (plataformas == true) {
                                                        let Telematic = JSON.parse(tmRenovacionActivacion(trama));
                                                        log.debug('Telematic-Entry', Telematic);
                                                        if (Telematic.results[0].code == 200) {
                                                            log.debug('Entry', 'If: ' + Telematic.results[0].code);
                                                            record.submitFields({
                                                                type: 'customrecord_ht_co_cobertura',
                                                                id: idCoberturaItem,
                                                                values: objValues,
                ...
                if (scriptContext.type === scriptContext.UserEventType.EDIT) {
                    if (objRecord.getValue('custbody_ht_ts_proceso_regularizacion') == 1) {
                        ...
                        for (let i = 0; i < numLines; i++) {
                            ...
                            let familiaArticulo = getParamFamiliaProductosArticuloOSDesinstalacion(_constant.Parameter.FAM_FAMILIA_DE_PRODUCTOS, articulo).valor;
                            log.debug('JCEC familiaArticulo ', familiaArticulo);
                            clienteCambio = objRecord.getSublistValue({ sublistId: 'item', fieldId: 'custcol_ht_os_cliente', line: i })
                            clienteMonitoreo = objRecord.getSublistValue({ sublistId: 'item', fieldId: 'custcol_ht_os_cliente_monitoreo', line: i }).length > 0 ? objRecord.getSublistValue({ sublistId: 'item', fieldId: 'custcol_ht_os_cliente_monitoreo', line: i }) : clienteCambio;
                            if (familiaArticulo) {
                                let customrecord_ht_ct_cobertura_transactionSearchObj = search.create({
                                    type: "customrecord_ht_co_cobertura",
                                    filters:
                                        [
                                            ["custrecord_ht_co_bien", "anyof", objRecord.getValue('custbody_ht_so_bien')],
                                            "AND",
                                            ["custrecord_ht_co_familia_prod", "anyof", familiaArticulo]
                                        ],
                                    columns:
                                        [
                        ...
                        if (aprobacionventa == _constant.Status.APROBADO && aprobacioncartera == _constant.Status.APROBADO) {
                            ...
                            for (var i = 0; i < jsonParameters.length; i++) {
                                ...
                                for (var j = 0; j < subArray.length; j++) {
                                    ...
                                    if (elemento.codigo_parametro === "ADP" && elemento.codigo_valor === "010") {
                            ...
                            if (resultado.length > 0) {
                                log.debug('Es Cambio de Propietario', 'Ingreso por cambio de propietario');
                                log.debug('clienteCambio', clienteCambio);
                                log.debug('clienteMonitoreo', clienteMonitoreo);
                                let resultBienPropietario = validateBienPropietario(objRecord.getValue('custbody_ht_so_bien'), clienteCambio);
                                if (resultBienPropietario == 0) {
                                    let dataUpdateCobertura = getCoberturasForCambioPropietario(objRecord.getValue('custbody_ht_so_bien'));
                                    log.debug('dataUpdateCobertura', dataUpdateCobertura);
                                    if (dataUpdateCobertura) {
                                        for (let index = 0; index < dataUpdateCobertura.length; index++) {
                                            ...
                                            record.submitFields({
                                                ...
                                                id: element,
                                                values: {
                                                    custrecord_ht_co_bien: objRecord.getValue('custbody_ht_so_bien'),
                        ...
                        //log.debug('jsonParameters', jsonParameters)
                        log.debug('Reversa-Reno-After', `${formulario} == ${_constant.Form.EC_ORDENES_DE_SERVICIOS} && ${typeof objRecord.getValue({ fieldId: 'void' })} != 'undefined' || ${objRecord.getValue({ fieldId: 'statusRef' })} == 'closed'`)
                        if (formulario == _constant.Form.EC_ORDENES_DE_SERVICIOS && (typeof objRecord.getValue({ fieldId: 'void' }) != 'undefined' || objRecord.getValue({ fieldId: 'statusRef' }) == 'closed')) {
                            //*REVERSA DE RENOVACIÓN ===========================================================
                            if (objRecord.getValue('custbody_ht_so_renovacion_aplicada') == true) {
                                let resultado = [];
                                for (var i = 0; i < jsonParameters.length; i++) {
                                    var subArray = jsonParameters[i];
                                    var contieneElemento = false;
                                    for (var j = 0; j < subArray.length; j++) {
                                        var elemento = subArray[j];
                                        if (elemento.codigo_parametro === "ADP" && elemento.codigo_valor === "004") {
                                            contieneElemento = true;
                                            break;
                                        }
                                    }
                                ...
                                for (const key in result) {
                                    if (result.hasOwnProperty(key)) {
                                        ...
                                        result[key].forEach(item => {
                                            log.debug('Debug1', `id_parametro: ${item.id_parametro}, codigo_parametro: ${item.codigo_parametro}, id_valor: ${item.id_valor}, codigo_valor: ${item.codigo_valor}`);
                                            let objUserAssetCommand = new Object();
                                            let dataUpdateCobertura = getDataUpdateCobertura(objRecord.getValue('custbody_ht_so_bien'), item.codigo_valor, objRecord.id);
                                            log.debug('dataUpdateCobertura', dataUpdateCobertura)
                                            if (dataUpdateCobertura.length > 0) {
                                                let active = dataUpdateCobertura[0].custrecord_ht_co_estado_cobertura == 1 ? true : false
                                                objUserAssetCommand.asset = dataUpdateCobertura[0].custrecord_ht_bien_id_telematic
                                                objUserAssetCommand.name = dataUpdateCobertura[0].custrecord_ht_bien_placa
                                                objUserAssetCommand.product_expire_date = dataUpdateCobertura[0].fechaReversadaUTC5Timezone
                                                objUserAssetCommand.active = active
                                                log.debug('objUserAssetCommand', objUserAssetCommand)
                                                if (dataUpdateCobertura[0].custrecord_ht_bien_id_telematic != null && dataUpdateCobertura[0].custrecord_ht_bien_id_telematic) {
                                                    setFechasCoberturasEnPlataforma(objUserAssetCommand);
                                                }
                                                log.debug('dataUpdateCobertura.id', dataUpdateCobertura[0].id);
                                                record.submitFields({
                                                    type: 'customrecord_ht_co_cobertura',
                                                    id: dataUpdateCobertura[0].id,
                                                    values: {
                                                        custrecord_ht_co_plazo: dataUpdateCobertura[0].plazoReversado,
                                                        custrecord_ht_co_coberturafinal: dataUpdateCobertura[0].fechaReversada
                                                    }
                                                })
                                            }
                                        });
    ...
    const getParameters = (item) => {
        let sql = "SELECT pa.id as id_parametro, " +
    ...
    const getDataUpdateCobertura = (bien, codigoFAM, osid) => {
        ...
        let sql = "SELECT co.id, co.custrecord_ht_co_estado_cobertura, co.custrecord_ht_co_coberturafinal, co.custrecord_ht_co_plazo, tr.id as detallecobertura, tr.custrecord_ht_ct_fecha_inicial, tr.custrecord_ht_ct_fecha_final, bn.custrecord_ht_bien_placa, bn.custrecord_ht_bien_id_telematic, " +
        ...
        let fechaReversada = getPlazoParaReversarCobertura(results[0].custrecord_ht_ct_fecha_inicial, results[0].custrecord_ht_ct_fecha_final, results[0].custrecord_ht_co_coberturafinal, results[0].custrecord_ht_co_plazo);
        let fechaReversadaformatDateUTC5Timezone = formatDateUTC5Timezone(fechaReversada[0]);
        results[0].fechaReversada = fechaReversada[0]
        results[0].plazoReversado = fechaReversada[1]
        results[0].fechaReversadaUTC5Timezone = fechaReversadaformatDateUTC5Timezone
        return results;
    }

    const getPlazoParaReversarCobertura = (fecha1, fecha2, fechaOriginal, plazo) => {
    ...
    const getCoberturasForCambioPropietario = (bien) => {
        let sql = "SELECT id FROM customrecord_ht_co_cobertura co WHERE custrecord_ht_co_bien = ?"
        let params = [bien];
        let results = query.runSuiteQL({ query: sql, params: params }).asMappedResults();
    ...
    const validateBienPropietario = (bien, propietario) => {
        let sql = "SELECT count(id) as conteo FROM customrecord_ht_record_bienes WHERE custrecord_ht_bien_propietario = ? AND id = ?"
    ...
    const crearCoberturaSinOt = (params) => {
        try {
            ...
            newCobertura.setValue({ fieldId: 'custrecord_ht_co_estado', value: _constant.Status.INSTALADO });
            ...
            let customrecord_ht_nc_servicios_instaladosSearchObj = search.create({
                ...
                columns:
                    [
                        ...
                        search.createColumn({ name: "item", join: "CUSTRECORD_NS_ORDEN_SERVICIO_SI", label: "Artículo" })
                    ]
            });
