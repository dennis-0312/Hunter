 ...
 */
 define(['N/search', 'N/record', 'N/runtime', 'N/log', 'N/file', 'N/task', "N/config", "N/format"], (search, record, runtime, log, file, task, config, format) => {
    // Schedule Report: Mensual: Libro Retenciones 4.1
    const execute = (context) => {
        try {
            const featureSubsidiary = runtime.isFeatureInEffect({ feature: 'SUBSIDIARIES' });
            const searchId = 'customsearch_pe_libro_rete_4_1';
    ...
    const getParams = () => {
        try {
            ...
            const filterSubsidiary = scriptObj.getParameter({ name: 'custscriptpe_subsidiary_ple_4_1' });
            const filterPostingPeriod = scriptObj.getParameter({ name: 'custscript_pe_period_ple_4_1' });
            const filterFormat = scriptObj.getParameter({ name: 'custscript_pe_formato_4_1' });
            const fileCabinetId = scriptObj.getParameter({ name: 'custscript_pe_filecabinetid_ple_4_1' });
    ...
    const createRecord = (logrecodId, featureSubsidiary, filterSubsidiary, filterPostingPeriod) => {
        try {
            ...
            recordlog.setValue({ fieldId: 'custrecord_pe_book_log', value: 'Libro de Retenciones 4.1' });
        ...
        try {
            for (let i in searchResult) {
                contentReport =
                    ...
                    searchResult[i].c10_importe_total + '|\n';
            }
    ...
    const createFile = (filterPostingPeriod, fedIdNumb, hasinfo, recordlogid, filterFormat, structuregbody, fileCabinetId) => {
        let typeformat;
        const header = '1 PERIODO,2 CUO,3 CORRELATIVO,4 FECHA DE EMISION DEL COMP DE PAGO O DOC,'+
                        '5 Tipo de Documento de identidad del Proveedor,'+
                        '6 Numero de RUC del Proveedor o Numero de documento de identidad,'+
                        '7 Apellidos y nombres  denominacion o razón social de proveedor,' +
                        '8 Monto Bruto,'+
                        '9 Retencion efectuada,'+
                        '10 Indicador de la Operacion'+
                        '\n';
        try {
           ...
           const periodostring  = periodoFinDate.getFullYear() + CompletarCero(2,periodoFinDate.getMonth() + 1) + '00';
            ...
            let nameReportGenerated = 'LE' + fedIdNumb + periodostring + '040100' + '00' + '1' + hasinfo + '11_' + recordlogid;
            if (filterFormat == 'CSV') {
    ...
    const CompletarCero = (numero,mes) => {
        log.error('mes',mes);
        mes = mes + '';
        var cant_num = mes.length;
        log.error('cant_num',cant_num);
        log.error('numero',numero);
        if(cant_num < numero){
            mes = '0' + mes;
        }
        return mes;
