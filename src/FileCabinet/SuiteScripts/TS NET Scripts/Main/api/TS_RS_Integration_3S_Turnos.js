...
define(["N/log", "N/search", "N/record", "N/query", "N/runtime"], (log, search, record, query, runtime) => {
    ...
    const _get = (scriptContext) => {
        ...
        try {
            ...
            let filter = [["custrecord_ht_ot_orden_servicio.class", "noneof", "@NONE@"], "AND", ["custrecord_ht_ot_orden_servicio.department", "noneof", "@NONE@"]];
            ...
            if (typeof scriptContext.turnodesde != "undefined" && typeof scriptContext.turnohasta != "undefined") {
                ...
                if (from == true && to == true) {
                    filter_Tecnico.push("AND", ["startdate", "within", newFechaDesde, newFechaHasta]);
                    param = true;
                    tecnico_param = true;
                } else {
                    return "Las fechas ingresadas no son válidas";
            ...
            if (typeof scriptContext.codigoClienteVehiculo != "undefined") {
                if (scriptContext.codigoClienteVehiculo.length != 0) {
                    id_cliente = getBuscarCliente(scriptContext.codigoClienteVehiculo);
            ...
            const consultaNro4 = search.create({ // consultaNro4 3S Turnos - DEVELOPER
            ...
            if (tecnico_param == true) {
                ...
                let searchResultCount_taller_param = Search_taller.runPaged().count;
                log.debug("searchResultCount_taller_param", searchResultCount_taller_param);
                ...
                //let searchResultCount_consultaNro4 = consultaNro4.runPaged().count;
                //log.debug("searchResultCount_consultaNro4", searchResultCount_consultaNro4);
                //let scriptObj = runtime.getCurrentScript();
                // log.debug('Remaining governance units entrada: ', scriptObj.getRemainingUsage());
                let myPagedData = consultaNro4.runPaged({ pageSize: 1000 });
                let arr = [];
                let contador = 1
                let datosCliente = getClienteDatos(id_cliente);
                myPagedData.pageRanges.forEach((pageRange) => {
                    let myPage = myPagedData.fetch({ index: pageRange.index });
                    myPage.data.forEach((result) => {
                        // let scriptObj2 = runtime.getCurrentScript();
                        // log.debug('Remaining governance units 2: ', `${scriptObj2.getRemainingUsage()} - Contador: ${contador}`);
    ...
    */
    ...
    const getBuscarCliente = (codigo) => {
        try {
            ...
            let searchResultCount_getBuscarCliente = customerSearchObj.runPaged().count;
            //log.debug("searchResultCount_getBuscarCliente", searchResultCount_getBuscarCliente);
            var arr = [];
    ...
    function condicionesScriptContext(scriptContext) {
        ...
        scriptContext?.ordenServicioSys ? scriptContext.ordenServicioSys.trim() : "";
    ...
    function obtenerOrdenServicio(scriptContext) {
        ...
        let DataUsuarioVehiculos = buscarPorCodigoClienteVehiculo(
            scriptContext.customer, scriptContext.iDVehiculo
        );
