...
define(['N/log', 'N/search', 'N/file', 'N/record'], function (log, search, file, record) {
    ...
    function getInputData() {
        let carpeta = './SUNAT';
        let nombreArchivo = 'AgenRet_TXT.txt';
        ...
        try {
            ...
            var vendorSearch = search.create({
                ...
                columns: ['custentity_pe_is_wh_agent', 'entityid', 'custentity_pe_document_number'],
            });
            ...
            while (page.length > 0) {
                page.forEach(function (result) {
                    ...
                    var ISRetencion = result.getValue('custentity_pe_is_wh_agent')

                    let data_vendor = {}
                    data_vendor.vendorId = vendorId
                    data_vendor.ISRetencion = ISRetencion
    ...
    function map(context) {
        try{
            ...
            var ISRetencion = parsedValue.ISRetencion;
            var vendorId = parsedValue.vendorId;
            let log_vendor = 'vendor [' + vendorId + '] --> ISRetencion['+ISRetencion+']'
            if (ISRetencion === false) 
            {
                ...
                vendorRecord.setValue({
                    fieldId: 'custentity_pe_is_wh_agent',
                    value: true
                });
                vendorRecord.save();
