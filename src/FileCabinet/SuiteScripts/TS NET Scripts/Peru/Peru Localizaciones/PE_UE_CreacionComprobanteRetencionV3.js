        ...
        try {
            ...
            if (context.type === context.UserEventType.CREATE || context.type === context.UserEventType.PAYBILLS || context.type === context.UserEventType.EDIT) {
                ...
                if (custrecord_pe_is_wht_agent == "true" || custrecord_pe_is_wht_agent == true) {
                    ...
                    let CodRetencionImpuestos = vendorRecord.getValue({ fieldId: 'custentity_4601_defaultwitaxcode' });
                    let peAgenteRetencion = vendorRecord.getValue({ fieldId: 'custentity_pe_is_wh_agent' });
                    let peBuenContriBuyente = vendorRecord.getValue({ fieldId: 'custentity_pe_is_good_contributor' });
                    let peSujetoRetencion = vendorRecord.getValue({ fieldId: 'custentity_pe_sujeto_retencion' });

                    log.debug('MSK', 'peAgenteRetencion: ' + peAgenteRetencion);
                    log.debug('MSK', 'peBuenContriBuyente: ' + peBuenContriBuyente);
                    log.debug('MSK', 'peSujetoRetencion: ' + peSujetoRetencion);
                    ...
                    if (CodRetencionImpuestos == COD_RETENCION_IMPUESTOS && peAgenteRetencion != true && peBuenContriBuyente != true && peSujetoRetencion == true) {
                        ...
                        //Inicio Cambio JCEC 20/08/2024
                        log.error('MSK', 'currency = ' + currency);
                        if (currency == 1) {
                            //actualizamos el total de facturas involucradas
                        ...
                        if (total_facturas_involucradas >= IMPORTE_MINIMO) {
                            ...
                            if (generaComprobanteRetencion) {
                                ...
                                vendorCredit.setValue('memo', 'Comprobante de Retención'); // descripcion
                                ...
                                vendorCredit.setValue('custbody_psg_ei_template', PE_EI_FEL_PLANTILLA); // PE_FEL_Template
                                log.error('1', '1')
                                vendorCredit.setValue('custbody_psg_ei_sending_method', PE_EI_FEL_SENDING_METHOD); // PE_FEL_Sending_Method
                                log.error('2', '2')
