        ...
        try {
            ...
            if (scriptContext.type === scriptContext.UserEventType.CREATE || scriptContext.type === scriptContext.UserEventType.COPY) {
                try {
                    ...
                    for (let i = 0; i < numLines; i++) {
                        ...
                        for (let j = 0; j < parametrosRespo.length; j++) {
                            ...
                            if (parametrosRespo[j][0] == _constant.Parameter.GOT_GENERA_SOLICITUD_DE_TRABAJO && parametrosRespo[j][1] == _constant.Valor.SI && !isGenerico) {
                                workOrder = _controller.parametros(_constant.Codigo_parametro.COD_GOT_GENERA_SOLICITUD_DE_TRABAJO, json);
                            ...
                            if (parametrosRespo[j][0] == _constant.Parameter.CCD_CONTROL_DE_CUSTODIAS_DE_DISPOSITIVOS) {
                                ccd = parametrosRespo[j][1]; itemCustodia.inventoryNumber = inventoryNumber;
                        ...
                        if (formulario == _constant.Form.EC_ORDENES_DE_SERVICIOS || formulario == _constant.Form.PE_ORDENES_DE_SERVICIOS) {
                    ...
                    if (adp == _constant.Valor.VALOR_006_MANTENIMIENTO_CHEQUEO_DE_DISPOSITIVO || adp == _constant.Valor.VALOR_002_DESINSTALACION_DE_DISP || adp == _constant.Valor.VALOR_007_CHEQUEO_DE_COMPONENTES || adp == _constant.Valor.VALOR_013_CHEQUEO_OTROS_PRODUCTOS) {
                        let out = 0;
                        ...
                        if (workOrder != 0) {
                            ...
                            if (esAlquiler == _constant.Valor.SI && adp == _constant.Valor.VALOR_002_DESINSTALACION_DE_DISP) {
                                log.error(
                                    "Entry",
                                    "Ingreso a envío de correo por geneación de desinstalación de alquiler"
                                );
                                let emailBody =
            ...
            if (scriptContext.type === scriptContext.UserEventType.EDIT || scriptContext.type === scriptContext.UserEventType.CREATE || scriptContext.type === scriptContext.UserEventType.COPY) {
                ...
                /*validacionDescuento != 0 && validacionGarantia &&*/ validacionNivelPrecio !=
                                ...
                                if (results.length > 0) {
                                    ...
                                    for (let i = 0; i < results.length; i++) {
                                        ...
                                        plataformas = _controller.getParameter(
                                            results[i].item,
                                            _constant.Codigo_parametro.COD_GPT_GENERA_PARAMETRIZACION_EN_TELEMATICS
                                        );
                                        plataformas = plataformas.idinterno == _constant.Valor.SI ? true : false;
                                        impulsaPX = _controller.getParameter(
                                            results[i].item,
                                            _constant.Codigo_parametro.COD_GPG_GENERA_PARAMETRIZACION_EN_GEOSYS
                                        );
                                        impulsaPX = impulsaPX.idinterno == _constant.Valor.SI ? true : false;
                                        t_PPS = _controller.getParameter(
                                            results[i].item,
                                            _constant.Codigo_parametro.COD_PPS_PEDIR_PERIODO_DE_SERVICIO
                                        );
                                        t_PPS = t_PPS.idinterno == _constant.Valor.SI ? true : false;
                                        ...
                                        let params2 = [familiaArtOS.idinterno, bien];
                                        let results2 = query
                    ...
                    if (scriptContext.type === scriptContext.UserEventType.EDIT) {
                        if (objRecord.getValue("custbody_ht_ts_proceso_regularizacion") == 1) {
                            let urlParams = {
