            ...
            try {
                ...
                for (i = 0; i < params.facturasSeleccionadas.length; i++) {
                    let loadRecord = record.load({
                        ...
                        id: params.facturasSeleccionadas[i].id,
                        isDynamic: true
                    });
					...
					for (var index = 0; index < linkscount - 1; index++) {
						...
						if(item == params.facturasSeleccionadas[i].items){
                           loadRecord.setSublistValue({ sublistId: 'item', fieldId: 'custcol_ht_fac_agru',value:facturaDirectaId ,line: index });
                        }
