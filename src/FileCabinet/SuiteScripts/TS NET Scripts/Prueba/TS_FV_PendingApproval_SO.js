...
define(['N/runtime', 'N/url', 'N/search', 'N/email'], function (runtime, url, search, email) {
    ...
    const onAction = (context) => {
        ...
        try {
            ...
            let subject = 'Factura de Venta ' + tranID + ' pendiente de aprobación por Cartera';
            let body = '<p>Número de Documento: ' + tranID + '<br>Cliente: ' + entity + '<br>Bien: ' + bien + '<br>Estado: Pendiente de Aprobación</p><p><br>Puedes revisarlo ingresando a: http://www.netsuite.com para aprobarlo.<br> </p>';
            ...
            EMPLOYID.forEach(function (employees) {
                sendEmail(employees.id, subject, body, '');
            })
        } catch (e) {
            log.error({
                title: `${FN} error`,
