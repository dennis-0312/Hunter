    ...
    const afterSubmit = (context) => {
        if (context.type === context.UserEventType.EDIT || context.type === context.UserEventType.CREATE) {
            try {
                ...
                var idItem = objRecord.getSublistValue({ sublistId: 'inventory', fieldId: 'item', line: 0 });
                var idItemText = objRecord.getSublistText({ sublistId: 'inventory', fieldId: 'item', line: 0 });
                log.debug('idItem', idItem);
                var avgunitcost = objRecord.getSublistValue({ sublistId: 'inventory', fieldId: 'avgunitcost', line: 0 });
                var adjustqtyby = objRecord.getSublistValue({ sublistId: 'inventory', fieldId: 'adjustqtyby', line: 0 });
                adjustqtyby = Number(adjustqtyby) * -1;
                log.debug('adjustqtyby', adjustqtyby);

                let newDetail = objRecord.getSublistSubrecord({ sublistId: 'inventory', fieldId: 'inventorydetail', line: 0 });
                ...
                var busqueda = search.lookupFields({ type: search.Type.ITEM, id: idItem, columns: ['custitem_ht_at_itemalquiler', 'custitem_ht_ar_tipoactivo'] });
                ...
                var busqueda_2 = search.lookupFields({ type: search.Type.ASSEMBLY_BUILD, id: idAssemblyBuild, columns: ['custbody_ht_ce_ordentrabajo'] });
                ...
                var busqueda_3 = search.lookupFields({ type: search.Type.SALES_ORDER, id: nameValueSalesOrder, columns: ['custbody_ht_os_tipoordenservicio', 'trandate'] });
